...
def generate_bar(data):
    ...
    open_ave = data['open'].mean()
    OHLC = pd.DataFrame(data = [[close, high, low, open_price, volume_ave, open_ave]], columns = ['close', 'high',
                                                                            'low', 'open', 'volume', 'open_ave'])

    return OHLC

def generate_avg_bar(data, barlength=120):
